import { useState, useMemo } from "react";
import {
  Radar,
  AlertTriangle,
  Sliders,
  CheckCircle2,
  XCircle,
  CloudSun,
  ShieldAlert,
  ArrowRight,
} from "lucide-react";

export default function ProactiveRiskRadar({
  trip,
  atRiskIds,
  onTriggerDisruption,
}) {
  const bookings = trip?.bookings || [];
  const [selectedRootId, setSelectedRootId] = useState(
    bookings[0]?.id || "bk_flight_out"
  );
  const [simulatedDelay, setSimulatedDelay] = useState(45);

  const rootBooking = useMemo(() => {
    return bookings.find((b) => b.id === selectedRootId) || bookings[0];
  }, [bookings, selectedRootId]);

  // Find immediate downstream dependents
  const directDependents = useMemo(() => {
    if (!rootBooking) return [];
    return bookings.filter((b) => b.dependsOn.includes(rootBooking.id));
  }, [bookings, rootBooking]);

  // Sensitivity calculation
  const simulationResults = useMemo(() => {
    if (!rootBooking) return [];

    return directDependents.map((child) => {
      const originalBuffer = child.bufferMinutes ?? 60;
      const remainingMargin = originalBuffer - simulatedDelay;
      const isBreached = remainingMargin <= 0;
      const isTight = remainingMargin > 0 && remainingMargin < 40;

      return {
        child,
        originalBuffer,
        remainingMargin,
        isBreached,
        isTight,
        breakPointMinutes: originalBuffer,
      };
    });
  }, [rootBooking, directDependents, simulatedDelay]);

  const earliestBreach = useMemo(() => {
    if (!simulationResults.length) return null;
    const sorted = [...simulationResults].sort(
      (a, b) => a.breakPointMinutes - b.breakPointMinutes
    );
    return sorted[0];
  }, [simulationResults]);

  return (
    <div className="space-y-6">
      {/* Top Banner: Proactive Risk Status */}
      <div className="rounded-3xl border border-border bg-surface p-6 shadow-sm">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 rounded-2xl bg-amber-dim flex items-center justify-center text-amber">
              <Radar className="h-5 w-5 animate-pulse" />
            </div>
            <div>
              <h3 className="font-display font-semibold text-base text-ink">
                Proactive Risk Radar
              </h3>
              <p className="text-xs text-ink-dim">
                Real-time connection vulnerability scanner & delay resilience simulator
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-amber-dim text-amber border border-amber/20">
              <AlertTriangle className="h-3.5 w-3.5" />
              {atRiskIds.length} Tight Margin Connection{atRiskIds.length !== 1 ? "s" : ""}
            </span>
          </div>
        </div>

        {/* Live Weather & Operations Advisory */}
        <div className="grid sm:grid-cols-2 gap-3 mt-5 pt-4 border-t border-border-light text-xs">
          <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-surface-sunk border border-border/60">
            <CloudSun className="h-4 w-4 text-blue shrink-0 mt-0.5" />
            <div>
              <span className="font-semibold text-ink">Regional Weather Outlook</span>
              <p className="text-ink-dim mt-0.5 leading-relaxed">
                Seasonal afternoon tropical squalls forecast across Denpasar & Ubud corridors. Speedboat and high-elevation transfers may experience 20–40m delays.
              </p>
            </div>
          </div>
          <div className="flex items-start gap-2.5 p-3 rounded-2xl bg-surface-sunk border border-border/60">
            <ShieldAlert className="h-4 w-4 text-amber shrink-0 mt-0.5" />
            <div>
              <span className="font-semibold text-ink">Airspace Congestion Advisory</span>
              <p className="text-ink-dim mt-0.5 leading-relaxed">
                BOM runway maintenance window between 12:00–16:00. Outbound sectors have 22% historical delay likelihood exceeding 30 minutes.
              </p>
            </div>
          </div>
        </div>
      </div>

      {/* Interactive "What-If" Delay Sensitivity Simulator */}
      <div className="rounded-3xl border border-border bg-surface p-6 shadow-sm space-y-5">
        <div className="flex items-center justify-between gap-3 flex-wrap">
          <div className="flex items-center gap-2">
            <Sliders className="h-4.5 w-4.5 text-coral" />
            <h4 className="font-display font-semibold text-sm text-ink">
              "What-If" Inbound Delay Sensitivity Simulator
            </h4>
          </div>
          <span className="text-xs text-ink-faint">
            Drag the slider to test cascade tipping points
          </span>
        </div>

        {/* Selector & Slider */}
        <div className="grid sm:grid-cols-2 gap-4 items-center bg-surface-sunk p-4.5 rounded-2xl border border-border/60">
          <div>
            <label className="block text-xs font-semibold text-ink-dim mb-1">
              Test Inbound Booking
            </label>
            <select
              value={selectedRootId}
              onChange={(e) => setSelectedRootId(e.target.value)}
              className="w-full bg-surface border border-border rounded-xl px-3 py-2 text-xs font-medium text-ink focus:outline-none focus:border-coral"
            >
              {bookings.map((b) => (
                <option key={b.id} value={b.id}>
                  {b.title} ({b.startTime})
                </option>
              ))}
            </select>
          </div>

          <div>
            <div className="flex items-center justify-between text-xs font-semibold mb-1">
              <span className="text-ink-dim">Simulate Inbound Delay</span>
              <span className="text-coral font-bold font-mono">+{simulatedDelay} minutes</span>
            </div>
            <input
              type="range"
              min="0"
              max="150"
              step="5"
              value={simulatedDelay}
              onChange={(e) => setSimulatedDelay(Number(e.target.value))}
              className="w-full accent-coral cursor-pointer"
            />
            <div className="flex justify-between text-[10px] text-ink-faint mt-1">
              <span>On Time (0m)</span>
              <span>+45m</span>
              <span>+90m</span>
              <span>+150m (Severe)</span>
            </div>
          </div>
        </div>

        {/* Dynamic Impact Readout */}
        <div className="space-y-3">
          <div className="flex items-center justify-between text-xs text-ink-dim font-semibold">
            <span>Downstream Connection Margin Analysis</span>
            {earliestBreach && (
              <span className="text-pink text-[11px]">
                Critical tipping point at +{earliestBreach.breakPointMinutes} mins
              </span>
            )}
          </div>

          {simulationResults.length === 0 ? (
            <p className="text-xs text-ink-faint py-3 text-center">
              This booking has no direct downstream dependents.
            </p>
          ) : (
            <div className="grid gap-3">
              {simulationResults.map(
                ({ child, originalBuffer, remainingMargin, isBreached, isTight }) => (
                  <div
                    key={child.id}
                    className={`p-4 rounded-2xl border transition-all ${
                      isBreached
                        ? "bg-pink-dim/40 border-pink/40"
                        : isTight
                        ? "bg-amber-dim/40 border-amber/40"
                        : "bg-surface-sunk border-border/80"
                    }`}
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-2.5">
                        {isBreached ? (
                          <XCircle className="h-4.5 w-4.5 text-pink shrink-0" />
                        ) : isTight ? (
                          <AlertTriangle className="h-4.5 w-4.5 text-amber shrink-0" />
                        ) : (
                          <CheckCircle2 className="h-4.5 w-4.5 text-teal shrink-0" />
                        )}
                        <div>
                          <h5 className="font-semibold text-xs text-ink">
                            {child.title} ({child.subtitle})
                          </h5>
                          <p className="text-[11px] text-ink-dim mt-0.5">
                            Scheduled: {child.startTime} · Original buffer: {originalBuffer}m
                          </p>
                        </div>
                      </div>

                      <div className="text-right shrink-0">
                        <span
                          className={`text-xs font-bold tabular-nums ${
                            isBreached
                              ? "text-pink"
                              : isTight
                              ? "text-amber"
                              : "text-teal"
                          }`}
                        >
                          {isBreached
                            ? `Breached by ${Math.abs(remainingMargin)}m`
                            : `Margin: ${remainingMargin}m left`}
                        </span>
                        <div className="text-[10px] text-ink-faint mt-0.5">
                          {isBreached
                            ? "Missed connection"
                            : isTight
                            ? "Connection high risk"
                            : "Safe connection"}
                        </div>
                      </div>
                    </div>

                    {/* Visual Progress Bar of buffer consumption */}
                    <div className="mt-3 pt-2 border-t border-border-light">
                      <div className="h-1.5 rounded-full bg-border overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-300 ${
                            isBreached
                              ? "bg-pink w-full"
                              : isTight
                              ? "bg-amber"
                              : "bg-teal"
                          }`}
                          style={{
                            width: isBreached
                              ? "100%"
                              : `${Math.max(10, Math.min(100, (remainingMargin / originalBuffer) * 100))}%`,
                          }}
                        />
                      </div>
                    </div>
                  </div>
                )
              )}
            </div>
          )}

          {/* Quick Action to Trigger the tested scenario */}
          {simulatedDelay > 0 && rootBooking && (
            <div className="pt-2 flex justify-end">
              <button
                onClick={() => onTriggerDisruption(rootBooking.id, "delay", simulatedDelay)}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-coral text-white text-xs font-semibold hover:brightness-105 transition shadow-sm"
              >
                Apply +{simulatedDelay}m Delay to Engine
                <ArrowRight className="h-3.5 w-3.5" />
              </button>
            </div>
          )}
        </div>
      </div>
    </div>
  );
}
